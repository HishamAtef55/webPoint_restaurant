...
$(document).ready(function()
    {
        $('#select_subgroup').on('change',function () {
            ...
            let sub_group = $(this).val();
                $.ajax({
                    'type':'POST',
                    'url':"<?php echo e(Route('search.select.item')); ?>",
                    'data':
                    {
    ...
    $(document).ready(function(){
        $('#extra_search').keyup(function ()
        {
            ...
            if(query != '')
            {
                ...
                $.ajax({
                    url:"<?php echo e(route('search.select.extra')); ?>",
                    method:'post',
...
$(document).on('click','#export_extra',function (e)
{
    ...
    console.log(branch);
    $.ajax({
        url      :"<?php echo e(route('export.extra')); ?>",
        method   :'post',
...
$(document).ready(function()
    {
        $('#select_items_sub').on('change',function () {
            ...
            if(item)
            {
                $.ajax({
                    'type':'POST',
                    'url':"<?php echo e(Route('get.item.extra')); ?>",
                    'data':{
                        '_token': "<?php echo e(csrf_token()); ?>",
                        'item': item},
                    success:function(data)
...
$(document).on('click', '.row_table_update', function (e) {
    ...
    let item       = $('#select_items_sub option:selected').val();
    $.ajax({
        url: "<?php echo e(route('update.export.extra')); ?>",
        method: 'post',
        data:
        {
            _token           : _token,
...
$.ajax({
    url: "<?php echo e(route('delete.export.extra')); ?>",
    method: 'post',
    data:
    {
        _token            : _token,
...
<?php /**PATH E:\MyWork\Res\webPoint\resources\views/includes/control/add_item_ajax.blade.php ENDPATH**/ ?>