<script>
    $(document).ready(function()
    {
        $('.select_Branch').on('change',function ()
        {
            let _token = $('input[name="_token"]').val();
            $('tbody').empty();
            let branch = $(this).val();
            if(branch)
            {
                $.ajax({
                    'type':'POST',
                    'url':"<?php echo e(Route('getDays.getDaysUsingBranch')); ?>",
                    'data':
                        {
                            'branch'  : branch,
                            '_token': "<?php echo e(csrf_token()); ?>",
                        },
                    success:function(data)
                    {
                        let html = '';
                        html += '<option value=""></option>';
                        for (var count = 0 ; count < data.data.length ; count ++)
                        {
                            html += '<option value="'+data.data[count].date+'">'+data.data[count].date+'</option>';
                        }
                        $('#select_menu').html(html);
                        getOptions ($('#select_menu'));
                    },
                });
            }
        });
    });
    $('#EmptyTable').on('click',function (e) {
        e.preventDefault();
        let _token = $('input[name="_token"]').val();
        let Branch = $('.select_Branch').val();
        let date = $('#select_menu').val();
        $.ajax({
            url: "<?php echo e(route('getDays.emptyTable')); ?>",
            method: 'post',
            enctype: "multipart/form-data",
            data:
                {
                    _token,
                    Branch,
                    date,
                },
            success: function (data) {
                if(data.status == true)
                {
                    Swal.fire({
                        position: 'center-center',
                        icon: 'success',
                        title: 'Your Tables Is Empty Data',
                        showConfirmButton: true,
                        timer: false,
                        focusConfirm : true
                    });
                }
            },
        });
    });

    $('#openDay').on('click',function (e) {
        e.preventDefault();
        let _token = $('input[name="_token"]').val();
        let Branch = $('.select_Branch').val();
        let date = $('#select_menu').val();
        $('#loading').addClass('show')

        $.ajax({
            url: "<?php echo e(route('getDays.openDay')); ?>",
            method: 'post',
            enctype: "multipart/form-data",
            data:
                {
                    _token,
                    Branch,
                    date,
                },
            success: function (data) {
                $('#loading').removeClass('show');
                if(data.status == true)
                {
                    Swal.fire({
                        position: 'center-center',
                        icon: 'question',
                        title: 'Your Day Is Open',
                        showConfirmButton: true,
                        timer: false
                    });
                }
                else if(data.status == false)
                {
                    Swal.fire({
                        position: 'center-center',
                        icon: 'error',
                        title: 'Check Orders This Table is not empty',
                        showConfirmButton: true,
                        timer: false
                    });
                }

            },
        });
    });

</script>
<?php /**PATH C:\xampp\htdocs\webpoint\resources\views/includes/control/open_day.blade.php ENDPATH**/ ?>