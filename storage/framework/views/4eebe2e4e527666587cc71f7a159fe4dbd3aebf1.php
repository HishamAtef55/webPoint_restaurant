...
function getItems(branchVal, itemsDiv) {
    $.ajax({
        url: "<?php echo e(route('getItemDetails')); ?>",
        method: 'post',
        data: {
            _token,
            branch: branchVal,
...
items.on('change', function () {
    ...
    materialArray = [];
    $.ajax({
        url: "<?php echo e(route('getDetails')); ?>",
        method: 'post',
        data: {
            _token,
            item: items.val(),
        ...
        success: function (data) {
            if (data.status == true) {
                ...
                data.data.forEach((details) => {
                    html += `<option value="${details.id}" data-price="${details.price}" >${details.name}</option>`
                });
                details.html(html)
                details.select2({
                    dir: "rtl",
...
details.on('change', function () {
    ...
    if ($(this).val()) {
        $.ajax({
            url: "<?php echo e(route('getMaterialsInDetails')); ?>",
            method: 'post',
            data: {
                _token,
                item: items.val(),
...
$(document).on('click', '.delete_Component', function () {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            calcPricePercent();
            $.ajax({
                url: "<?php echo e(route('deleteComponent')); ?>",
                method: 'post',
                data: {
                    _token,
...
saveComponent.on('click', function () {
    $.ajax({
        url: "<?php echo e(route('saveDetailsComponent')); ?>",
        method: 'post',
        data: {
            _token,
            branch: branch.val(),
...
DetailsWithoutMaterials.on('click', function () {
    let title = $(this).text();
    $.ajax({
        url: "<?php echo e(route('DetailsWithoutMaterials')); ?>",
        method: 'post',
        data: {
            _token,
            branch: branch.val(),
                        ...
                        },
                        {
                            ...
                            customize: function (doc,s) {
                                doc.content[1].table.body.forEach(row => {
                                    if (row[row.length - 1].text === 'color') {
                                        row.forEach(col => col.fillColor = '#c5d7f2')
                                    }
                                    row.pop()
                                })
...
printDetails.on('click', function () {
    let title = $(this).text();
    $.ajax({
        url: "<?php echo e(route('printDetails')); ?>",
        method: 'post',
        data: {
            _token,
            branch: branch.val(),
...
$('#save_transfer').on('click', function () {
    ...
    $.ajax({
        url: "<?php echo e(route('transfierMaterialDetails')); ?>",
        method: 'post',
        data: {
            _token,
...
<?php /**PATH C:\xampp\htdocs\web_point\resources\views/includes/stock/Stock_Ajax/componentDetailsItems.blade.php ENDPATH**/ ?>