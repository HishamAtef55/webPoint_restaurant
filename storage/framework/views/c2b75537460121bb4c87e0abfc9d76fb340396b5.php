...
$(document).ready(function(){
    ...
    $('#editable').Tabledit({
        url:'<?php echo e(route("expenses.action")); ?>',
        dataType:"json",
        columns:{
            identifier:[[0, 'id'],[1,'branch_id']],
            editable:[[2, 'name']],
        },
        restoreButton:false,
    ...
    $('#search').keyup(function ()
    {
        ...
        if(query != '')
        {
            ...
            $.ajax({
                url:"<?php echo e(route('expenses.search')); ?>",
                method:'post',
                ...
                success:function(data)
                {
                    ...
                    for(var count = 0 ; count < data.length ; count ++)
                    {
                        ...
                        html += '<td class="">';
                            ...
                            html += '<input id="branch_id" class="tabledit-input tabledit-identifier" type="hidden" name="branch_id" value="'+data[count].branch.name+'">';
                        html += '</td>';

                        html += '<td class="tabledit-edit-mode">';
                            html += '<span class="tabledit-span tabledit-identifier">'+data[count].title+'</span>';
                            html += '<input type="text" style="display: none;" disabled class="tabledit-input form-control input-sm" name="name" value="'+data[count].title+'">';
                        html += '</td>';
    ...
    $(document).on('click','#save_menu',function (e)
    {
        ...
        $.ajax({
            type    : 'POST',
            url     :"<?php echo e(route('expenses.save')); ?>",
            method  : 'post',
...
<?php /**PATH C:\xampp\htdocs\webpoint\resources\views/includes/control/Expenses.blade.php ENDPATH**/ ?>