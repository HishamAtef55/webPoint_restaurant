...
<script>
    ...
    let from_branch = $('#from_branch_id');
    let to_branch = $('#to_branch_id');
    let from_section = $('#from_section_id')
    let to_section = $('#to_section_id')
    let from_store = $('#from_store_id');
    let to_store = $('#to_store_id');
    ...
    $(document).ready(function() {
        ...
        function changePurchasesMethod() {
            const firstFromBranchOptionValue = from_branch.find("option:first").val();
            const firstToBranchOptionValue = to_branch.find("option:first").val();
            const firstFromStoreOptionValue = from_store.find("option:first").val();
            const firstToStoreOptionValue = to_store.find("option:first").val();
            const purchasesMethod = document.querySelector(
                'input[name="purchases_method"]:checked'
            )?.value;

            if (purchasesMethod === "sections") {
                console.log(firstFromBranchOptionValue)
                console.log(firstToBranchOptionValue)
                console.log(firstFromStoreOptionValue)
                console.log(firstToStoreOptionValue)
                from_section.empty()
                ...
                to_section.empty()
                    .val("<option selected disabled>اختر القسم </option>")
                    .trigger("change");
                from_branch.val(firstFromBranchOptionValue).change();
                to_branch.val(firstToBranchOptionValue).change();
                document
                ...
                from_store.val(firstFromStoreOptionValue).change();
                to_store.val(firstToStoreOptionValue).change();
                document
                    .querySelectorAll(".branch-sec")
                    .forEach((el) => el.classList.add("d-none"));
                document
        ...
        from_store.on("change", getMaterials)

        from_section.on("change", getSectionsMaterials)

        from_branch.on("change", getFromSections);

        to_branch.on("change", getToSections);


        /*
         * getFromSections
         */
        function getFromSections() {
            const branchSelectId = from_branch.val();
            if (!branchSelectId) {
                return;
            }
         ...
         * getToSections
         */
        function getToSections() {
            const branchSelectId = to_branch.val();
            if (!branchSelectId) {
                return;
            }
            if (preventChangeEvent) return;
        ...
        function getMaterials() {
            const storeSelectId = from_store.val();
        ...
        function getSectionsMaterials() {
            const sectionSelectId = from_section.val();
         ...
         * displaySections
         */
        function displayFromSections(sections) {
            let container = $("#from_section_id");
            let html = '';
            if (!sections.length) {
                html += `<option value="">لاتوجد اقسام</option>`;
            } else {
                html = `<option value="" disabled selected>اختر القسم</option>`;
         ...
         * displaySections
         */
        function displayToSections(sections) {
            let container = $("#to_section_id");
            let html = '';
            if (!sections.length) {
                html += `<option value="">لاتوجد اقسام</option>`;
            } else {
        ...
        function setData(method = null) {
            let materialArray = [];
            let transfer_type = $('input[name="purchases_method"]:checked').val();
            ...
            if (transfer_type == "sections") {
                formData.append("from_section_id", from_section.val());
                formData.append("to_section_id", to_section.val());
            } else if (transfer_type === "stores") {
                formData.append("from_store_id", from_store.val());
                formData.append("to_store_id", to_store.val());
            }
