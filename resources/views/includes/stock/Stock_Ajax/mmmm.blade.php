            ...
            $.ajax({
                ...
                success: function(data) {
                    if (data.status == true) {
                        ...
                        if (branchCheck.is(':checked')) {
                            branchCheck.parents('.form-check').next('.branch-sections')
                                .html(html)
                            branchCheck.parents('.form-check').next('.branch-sections')
                                .removeClass('d-none')
                        } else {
            ...
            sectionsArray.forEach(section => {
                if (section.id == branchId) {
                    if ($(this).is(':checked')) {
                        ...
                        section.sections = section.sections.filter(section => section.id !=
                            sectionId)
                    }
                }
            });
            ...
            if (requestCheck) {
                $.ajax({
                    ...
                    data: {
                        ...
                        expire: expire.val() || 0,
                        dailyInventory: dailyInventory.is(':checked') ? dailyInventory.val() :
                            0,
            ...
            getData('get_material_in_ul', $(this).attr('data-id'), function(data) {
                ...
                data.sections.forEach(section => {
                    if (branchArr.indexOf(section.branch) == -1) {
                        $(`input#branch_${section.branch}[name="branch"]`).prop(
                            'checked', true).change();
                    }
                    branchArr.push(section.branch)
                    setTimeout(() => {
                        $(`input#section_${section.section}`).prop('checked',
                            true).change();
                    }, 800)
                });
